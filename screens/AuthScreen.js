...
export default function AuthScreen({ hasStoredSession = false, onBiometricUnlock, onFullSignOut }) {
  ...
  const signInWithBiometric = async () => {
    if (!hasStoredSession) {
      setError("Sign in with password first to enable Face ID / Touch ID.");
      return;
    }
    const result = await LocalAuthentication.authenticateAsync({
      promptMessage: "Sign in to Ojas Bloom Studio",
    ...
    <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={s.wrap}>
      <Animated.View style={[s.content, { opacity: fadeAnim, transform: [{ translateY: slideAnim }] }]}>
        ...
        <View style={[s.card, SHADOW.md]}>
          ...
          {/* Biometric sign-in button — always shown on native if hardware available */}
          {biometricAvailable && (
